...
<rss version="2.0"  xmlns:cc="http://web.resource.org/cc/" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:media="http://search.yahoo.com/mrss/" xmlns:content="http://purl.org/rss/1.0/modules/content/"  xmlns:podcast="https://podcastindex.org/namespace/1.0"  xmlns:googleplay="http://www.google.com/schemas/play-podcasts/1.0" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    <channel>
        <title>
            {{ $podcaster->artist_name}}
        </title>
        <pubDate>{{ $podcaster->created_at }}</pubDate>
        <language>en</language>
        <link>
            {{request()->url()}}
        </link>
            <itunes:wrapper_type> {{$podcaster->wrapper_type}}</itunes:wrapper_type>
            <itunes:artist_type> {{$podcaster->artist_type}}</itunes:artist_type>
            <itunes:artist_id> {{$podcaster->artist_id}}</itunes:artist_id>
            <itunes:artist_name> {{$podcaster->artist_name}}</itunes:artist_name>
            <itunes:artist_link_url> {{$podcaster->artist_link_url}}</itunes:artist_link_url>
            <itunes:primary_genre_name> {{$podcaster->primary_genre_name}}</itunes:primary_genre_name>
            <itunes:primary_genre_id> {{$podcaster->primary_genre_id}}</itunes:primary_genre_id>
            <itunes:isset_artist_id> {{$podcaster->isset_artist_id}}</itunes:isset_artist_id>
            <itunes:artwork_url30> {{$podcaster->artwork_url30}}</itunes:artwork_url30>
            <itunes:artwork_url60> {{$podcaster->artwork_url60}}</itunes:artwork_url60>
            <itunes:artwork_url100> {{$podcaster->artwork_url100}}</itunes:artwork_url100>
            <itunes:artwork_url_600> {{$podcaster->artwork_url_600}}</itunes:artwork_url_600>

        @foreach($podcaster->podcasts as $podcast)
            <item>
                ...
                @foreach($podcast->episodes as $episode)
                    <item>
                        ...
                        <itunes:podcast_id> {{$episode->podcast_id}}</itunes:podcast_id>
                        <itunes:podcaster_id> {{$episode->podcaster_id }}</itunes:podcaster_id>
                    </item>
                @endforeach
            </item>
        @endforeach
